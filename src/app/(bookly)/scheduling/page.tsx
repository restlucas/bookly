...
export default function Scheduling() {
  ...
  const fetchScheduling = useCallback(
    ...
    [selectedStatus.personal, selectedStatus.professional, user.id],
  )

  // Watching professional status filter change to fetch professional scheduling
  useEffect(() => {
    if (user.userType)
      if (user.userType.slug === 'professional') fetchScheduling('professional')
  }, [user, selectedStatus.professional, fetchScheduling])

  /// Watching personal status filter change to fetch personal scheduling
  useEffect(() => {
    if (user.userType) if (user.userType.slug) fetchScheduling('personal')
  }, [user, selectedStatus.personal, fetchScheduling])
